  ...
  <div>
    <v-row no-gutters>
      <v-img
        height="300"
        width="100%"
        style="position: absolute; opacity: 0.45; z-index: 1"
        :src="courseList[this.$route.params.id - 1].image"
      >
        <div class="fill-height bottom-gradient"></div>
      </v-img>
      <v-col cols="12" style="padding: 160px 100px 100px 100px; z-index: 2;">
        <v-row>
          <v-col cols="3">
            <v-card
              flat
          ...
          <v-col cols="6" class="mt-2">
            <h2>{{ courseList[this.$route.params.id - 1].title }}</h2>
            <div>
              <span class="primary--text font-weight-bold">{{
                courseList[this.$route.params.id - 1].price
              }}</span>
              <div>
                <span>{{
                  courseList[this.$route.params.id - 1].sub_title
                }}</span>
              </div>
            </div>
          </v-col>
          <v-col cols="3" style="padding: 100px 0px 0px 0px">
            <v-btn
              fab
              width="100"
              height="100"
              color="success elevation-8"
              :to="{
                path: '/play-course/' + this.$route.params.id,
              }"
              ><div><span>Mulai</span></div>
              <v-icon>mdi-play</v-icon></v-btn
            >
          </v-col>
        </v-row>
        <v-row no-gutters>
          <h2 class="mt-12">Silabus</h2>
        </v-row>
        <v-row no-gutters>
          <v-col cols="12">
            <v-data-table
...
export default {
  ...
  mounted() {
    ...
    // console.log("images ", this.courseList[course_id].image);
  },
  data: () => ({
    course_id: "",
    courseList: [
      {
        ...
        sub_title:
          "Belajar dasar dasar dari html atau hyper text markup language",
        image:
    ...
    headers: [
      { text: "Materi", value: "materi" },
      { text: "Waktu Belajar", value: "waktu" },
      { text: "Mentor", value: "mentor" },
    ],
    courses: [
      {
        materi: "Pengantar HTML",
        waktu: "10 Menit",
        mentor: "Nur Hidayah",
      },
      {
      ...
      {
        materi: "Membangun Project HTML",
        waktu: "60 Menit",
        mentor: "Nur Hidayah",
      },
      {
        materi: "Submission 1",
        waktu: "Sampai Batas Waktu Kelas",
        mentor: "Rifqi Fathurrahman",
      },
      {
        materi: "Penutupan",
        waktu: "-",
